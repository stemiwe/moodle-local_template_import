...
class report_table extends table_sql {
    ...
    /**
     * Turns data from column "templatefileid" into user-friendly html link.
     *
     * @param \stdClass $data The row data.
     * @return string Link to backup area of file with filename as text.
     */
    public function col_templatefileid($data) {
        global $DB;
        if ($data->templatefileid) {
            $file = $DB->get_record('files', array('id' => $data->templatefileid));
            $link = '<a href="';
            $link .= new moodle_url('/backup/restorefile.php', array('contextid' => $file->contextid));
            $link .= '">';
            $link .= $file->filename;
            $link .= "</a>";
            return $link;
